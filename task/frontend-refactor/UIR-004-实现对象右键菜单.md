...
- 实际修改文件：
  - `mactfsui/app/components/app/context-menu.tsx`
...
- 实际实现内容：
  ...
  - Pending Changes Included / Excluded 项接入右键菜单。
  - 目录对比结果项接入右键菜单。
  - pending edit 文件禁用直接 Get Latest，pending add 文件隐藏服务器比较入口。
  - Checkin 仍保留在右侧 Changes 面板。
- 测试结果：
  - 已执行 `pnpm typecheck`，通过。
- 遗留问题：
  - 左侧目录树右键菜单第一版未单独展开实现，当前对象操作主要覆盖中间列表、目录对比和 Pending Changes。
