...
普通用户无需自行构建，直接到 [GitHub Releases](https://github.com/Farewell0375/MacTFS/releases/latest) 下载对应芯片的安装包：

| 安装包 | 适用机型 |
| --- | --- |
| `MacTFS-<版本>-arm64.dmg` | Apple Silicon（M 系列芯片） |
| `MacTFS-<版本>-x64.dmg` | Intel 芯片 |

打开 DMG，把 `MacTFS.app` 拖进「应用程序」即可。

### 首次打开：应用未签名怎么办

本应用**未做 Apple 代码签名 / 公证**，首次打开 macOS 的 Gatekeeper 会拦截，提示「无法验证开发者」或「"MacTFS" 已损坏，无法打开」。这是未签名应用的正常现象，二选一解除即可：

**方法一 · 图形界面**

1. 双击 `MacTFS.app`，弹窗里先点「完成 / 取消」。
2. 打开 **系统设置 → 隐私与安全性**，下拉到「安全性」区域，会看到一条「已阻止使用 MacTFS」的提示，点 **仍要打开**。
3. 再次确认，之后即可正常启动。

**方法二 · 终端去掉隔离属性（推荐，最省事）**

把应用拖进「应用程序」后，在终端执行下面这条命令，去掉 macOS 给下载文件打的隔离标记（quarantine）：

```bash
xattr -dr com.apple.quarantine /Applications/MacTFS.app
```

执行后直接双击即可打开。若仍提示「已损坏」，再用这条（会清除全部扩展属性）：

```bash
sudo xattr -cr /Applications/MacTFS.app
```

> 这些步骤只是绕过「未签名」拦截，应用本身不会因此被修改。介意签名/公证的话，需要 Apple 开发者账号自行签名。
