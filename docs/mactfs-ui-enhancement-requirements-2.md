...
### R11 访达打开与添加本地文件（FE-025，P0，第二批追加）

- 用户反馈原文：缺少右键文件或文件夹时「在访达中打开」（进入到访达的本目录）；
  右键文件夹或在中间区域右键时，缺少添加文件的功能（本地新增的文件加入 TFS 控制）。
- 在访达中打开：
  - 文件：访达中显示并选中该文件（`shell.showItemInFolder`）
  - 目录：直接进入该目录（`shell.openPath`）
  - 仅 Electron 环境可用；未映射对象置灰
- 添加本地文件（修订：按用户反馈改为扫描式，不手动找文件）：
  - 目录右键与中间区域空白处右键提供「添加本地文件…」
  - 自动对该目录执行目录对比（`POST /api/compare/folder` 递归），筛出 `localOnly`
    （本地新增、服务器不存在）的文件 / 目录
  - 弹窗列表勾选（默认全选，支持全选 / 清空），确认后批量
    `POST /api/files/add` 产生 pendingAdd 挂起更改，完成后刷新
- 中间区域空白处右键 = 当前目录的完整右键菜单（与目录树右键一致）
